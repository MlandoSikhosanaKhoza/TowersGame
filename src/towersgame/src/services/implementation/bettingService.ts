...
export class BettingService implements IBettingService {
    ...
    getTop5RewardList = (difficulty: Difficulty, rowNumber: number, noOfRows: number): number[] => {
        const deduction = (rowNumber - 4);
        const toValue = deduction < 0 ? 0 : deduction;
        console.log(toValue);
        const rewardList = [];
        for (let i = rowNumber; toValue <= i; i--) {
            rewardList.push(ScoreSystemHelper.getRewardMultiplierFor(difficulty, i, noOfRows));
        }
        return rewardList;
    }
    ...
    */
    calculateCollectionAmount = (difficulty: Difficulty, bettingAmount: number, rowNumber: number, noOfRows: number): number => {
        ...
        return ScoreSystemHelper.getRewardMultiplierFor(difficulty, rowNumber, noOfRows) * bettingAmount;
    }
